...
or an empty value like None or null, depending on your language. You 
may assume the argument is non-negative.
...
long int findNextSquare(long int sq) {
  long int sqn = sqrt(sq);
  return sq == pow(sqn, 2) ? pow(sqrt(sq) + 1, 2) : -1;
}
