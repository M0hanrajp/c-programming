/* Imagine you start on the 5th floor of a building, then travel down to the 
 * 2nd floor, then back up to the 8th floor. You have travelled a total of 3 + 6 = 9 floors of distance.
...
unsigned elevator_distance (size_t length, const int floors[length]) {
  ...
  for(size_t i = 1; i < length; i++) {
    d += floors[i] < floors[i - 1] ? floors[i - 1] - floors[i] 
                                   : floors[i] - floors[i - 1];
  }
	return d;
}
